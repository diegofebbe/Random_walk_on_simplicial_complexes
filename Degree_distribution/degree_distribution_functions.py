...
def game_links_and_tetrahedra(l, t, labels_l, labels_t):
    # Calcola il numero di nodi già presenti
    n = labels_l.max() + 1  # Più efficiente di set-comprehension

    # Scelta del triangolo da cui crescere
    p = t / np.sum(t) if np.sum(t) > 0 else np.array([1.0])
    idx = np.random.choice(len(t), p=p)
    triangle = labels_t[idx]

    # Costruzione dei 3 link del triangolo
    selected_links = {
    ...
    # Mappa link → indice (solo una volta, può anche essere costruita all'esterno e riusata!)
    link_index_map = {tuple(link): i for i, link in enumerate(labels_l)}

    # Aggiorna i gradi dei link selezionati
    for link in selected_links:
        idx_l = link_index_map.get(link)
        if idx_l is not None:
            l[idx_l] += 1

    # Incrementa il grado del triangolo selezionato
    t[idx] += 1

    # Costruzione nuovi link e triangoli
    new_node = n
    new_links = []
    new_triangles = []

    for node in triangle:
        # Nuovo link (node, new_node)
    ...
    # Aggiunta batch a labels_l e labels_t
